...
class ZKLib {
    ...
    async createSocket(cbErr, cbClose){
        try{
            ...
            if(err.code !== ERROR_TYPES.ECONNREFUSED && (!err.message || !err.message.includes('TIMEOUT'))){
                console.log('⚠️ TCP connect failed with error, falling back to UDP. Error:', err.message || err.code);
            }
