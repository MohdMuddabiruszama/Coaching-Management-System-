  ...
  createSocket(cbError, cbClose) {
    return new Promise((resolve, reject) => {
      ...
      if (this.timeout) {
        ...
        // Without a 'timeout' handler the socket stalls silently instead of failing.
        // Destroy it so the error propagates and we retry cleanly.
        this.socket.on('timeout', () => {
          this.socket && this.socket.destroy(new Error('SOCKET_TIMEOUT'))
        })
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket.write(msg, null, async (err) => {
        if (err) {
          ...
          }, connect ? 15000 : this.timeout)  // 15s for CMD_CONNECT — ZKTeco Wi-Fi devices need extra time
