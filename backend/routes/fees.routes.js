...
router.post("/pay", verifyToken, allowRoles("admin", "student", "manager"), checkManagerPermission("fees.create"), checkFeatureAccess("feature_fees"), validate(feesValidator.recordPayment), feesController.recordPayment);
router.get("/payments", verifyToken, allowRoles("admin", "manager"), checkManagerPermission("fees.read"), checkFeatureAccess("feature_fees"), validate(feesValidator.getPayments), feesController.getAllPayments);
router.get("/payment/:student_id", verifyToken, allowRoles("admin", "faculty", "student", "manager"), checkManagerPermission("fees.read"), checkFeatureAccess("feature_fees"), validate(feesValidator.getStudentPayments), feesController.getStudentPayments);
